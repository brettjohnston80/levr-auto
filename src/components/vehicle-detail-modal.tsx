...
export function VehicleDetailModal({
  ...
  // Scrolling and centering are deliberately on two SEPARATE elements
  // (2026-09-02, real bug reported by external mobile testers). They used
  // to be one: `fixed inset-0 flex items-center justify-center
  // overflow-y-auto` on the backdrop. When the card is taller than the
  // viewport, `items-center` centers it by pushing its top edge ABOVE the
  // scroll container's top -- into negative scroll space that scrollTop
  // can never reach (it floors at 0), so the card's heading and its
  // absolutely-positioned close button were rendered but permanently
  // unreachable. Desktop hid this because the body grid is grid-cols-1
  // lg:grid-cols-2 (below) and fits in two columns; mobile stacks title,
  // price, inventory count, fit bullets, the full 9-dimension breakdown,
  // photo AND video into one column, comfortably overflowing.
  //
  // The fix is the standard two-element pattern: overflow-y-auto stays on
  // the fixed backdrop, and centering moves to an inner `min-h-full flex
  // items-center` wrapper. Short content still centers exactly as before;
  // tall content now grows the scroll area downward from a reachable top.
  // The wrapper is a plain child, so a click on it still bubbles to the
  // backdrop's onClose -- click-outside-to-close is unchanged.
  return createPortal(
    <div
