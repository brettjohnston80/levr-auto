...
/**
 * Switches a customer's search to a new make/model — never a silent edit.
 * The actual insert-new-row + mark-old-row-superseded pair happens atomically
 * in the switch_customer_search DB function (see migrations), since two
 * related writes need to succeed or fail together and supabase-js has no
 * multi-statement transaction. Ownership is checked here first; the DB
 * function re-checks the not-already-switched guard under a row lock, so a
 * race between this check and the RPC call can't double-switch a search.
 *
...
export async function switchCustomerSearch(formData: FormData): Promise<SwitchSearchResult> {
  ...
  const { data: oldSearch, error: oldSearchError } = await admin
    ...
    .select("id, customer_id, search_status, superseded_by_id")
    .eq("id", oldSearchId)
    .maybeSingle();
  ...
  if (oldSearch.customer_id !== user.id) {
