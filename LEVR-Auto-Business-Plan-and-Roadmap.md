...
**Aug 18, 2026 — Inbound dealer-offer parsing (Claude API) built and verified end-to-end — build order item 7, done.** Investigation before building found the manual offer-logging path already fully working (`/internal/outreach`'s "Log an offer" form) and a real landmine worth designing around: the guarantee logic and customer dashboard never check `qualifying_offers.status`, only `is_below_msrp`/`delivered_at`/etc. — so any AI-parsed draft written directly into that table, even behind a new status flag, would have shown up on a customer's dashboard and started counting toward the guarantee the moment they next loaded the page, silently skipping the review step. Solved by never writing anywhere except through the existing single confirm action: parsing (text paste or a PDF upload — both handled via Claude reading real dealer replies) only pre-fills the existing manual form, and nothing persists until an agent reviews and clicks "Save offer." No staging table, no audit log of parse attempts — confirmed offers and, when a PDF was attached, the offer-sheet document are the only things that ever get written.

MSRP stays entirely human-entered — never in Claude's extraction schema — since dealers state their own price, not the vehicle's MSRP, and getting that wrong would directly corrupt a guarantee determination. The offer-sheet PDF itself is stored (reusing the existing `documents` table/Storage bucket, a new `offer_sheet` type) and shown to the customer via a signed URL, then cleaned up by a new daily cron once the search reaches a terminal status (cancelled, purchased, or switched) — deliberately never on `guarantee_status` resolving, since a search stays active through Day 60 regardless of that outcome and could easily still be mid-close right after. Soft delete: the real file is deleted, the `documents` row stays as a historical record.

Verified against real data throughout: real Claude API calls against both a pasted dealer email and a real generated PDF, both parsed correctly (dealer name, contact, price with itemized fees correctly separated out); the atomic save producing correct `qualifying_offers`/`offer_addons`/`documents` rows; the signed URL fetched directly and confirmed byte-for-byte identical to the source file; the deletion cron confirmed against the real Storage API (a same-URL re-fetch briefly still returning 200 turned out to be a CDN cache hit, not a failed deletion — confirmed via the Storage list API directly). Full detail in CLAUDE.md.
